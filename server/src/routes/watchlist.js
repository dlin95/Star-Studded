...
module.exports = (db) => {
  ...
  router.post('/watchlist', (req, res) => {
    ...
    db.query(selectQueryString, selectQueryParams).then((result) => {
    ...
    const queryParams = [newWatchList.user_id, newWatchList.movie_id, newWatchList.poster_path, newWatchList.title, newWatchList.vote_average, newWatchList.release_date];
  ...
  router.delete('/watchlist/:id/delete', (req, res) => {
    ...
    db.query(`DELETE FROM watchlist WHERE movie_id = ${movie_id}::varchar`)
      .then((result) => {
